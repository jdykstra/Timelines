...
class WindowState extends Object implements Serializable {
	...
	// 	Constants ----------------------------------------------------------------------
	protected static final int PORTABLE_STREAM_VERSION_1 = 1;
	protected static final int PORTABLE_STREAM_VERSION_2 = 2;
	...
	public int iCyclicYear;					//  Cyclic year, meaningful iff iCyclicView is true
	...
	public WindowState(DefinedCategorySet categories, DataInputStream is)
		...
		switch (version){
			case PORTABLE_STREAM_VERSION_1:
				...
				iCyclicYear = 2022;  // Cyclic view hardwired to 2022 before portable stream version 2
				break;
			case PORTABLE_STREAM_VERSION_2:
				iWinPosition = new Point(is.readInt(), is.readInt());
				iWinSize = new Dimension(is.readInt(), is.readInt());
				iResolution = is.readInt();
				iScrollPosition = is.readLong();
				iShownCats = categories.getSharedMemberSet(is);
				iCyclicView = is.readBoolean();
				iCyclicYear = is.readInt();
				break;
			default:
				throw new FileFormatError("Unsupported WindowState file version (" + version + ")");
		}
	}
	...
	public void writeTo(DataOutputStream os) 
							throws IOException {
		os.writeShort(PORTABLE_STREAM_VERSION_2);
